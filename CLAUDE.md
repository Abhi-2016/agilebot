...
| 2026-09-07 | Azure Bot Service setup, HMAC JWT verification, ConnectorClient reply pattern, next-gen JIRA API compatibility, null-safe API field access, Single Tenant bot auth | Week 2 Days 6-7 complete. Azure Bot registered, Teams channel wired, ngrok tunnel established, HMAC signature verification added, ConnectorClient reply loop wired. Full bot loop verified via Azure Web Chat. Key bugs: (1) `openSprints()` JQL unsupported in next-gen Jira — fixed with Agile board API sprint resolution; (2) `priority` field null in next-gen projects — fixed with `.get()` fallback; (3) Single Tenant bot needs `channel_auth_tenant` in credentials — without it `KeyError: access_token`. |
