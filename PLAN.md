...
| 2026-09-07 | JIRA project type compatibility | Replaced `openSprints()` JQL with Agile board API sprint resolution | ✅ Right | `openSprints()` only works in classic (company-managed) Jira projects. Next-gen (team-managed) projects require fetching the active sprint ID via `/rest/agile/1.0/board/{id}/sprint?state=active` then querying `/rest/agile/1.0/sprint/{id}/issue`. External APIs have undocumented project-type variations — always verify against your actual instance. |
| 2026-09-07 | Null-safe field access | Fixed `fields["priority"]["name"]` crash when priority is None | ✅ Right | JIRA fields that appear mandatory in the UI (priority, assignee) are often null in the API for next-gen projects. Never use direct key access on nested API response fields — always use `.get()` with a fallback. One missing field silently breaks the entire tool result. |
| 2026-09-07 | Bot auth for Single Tenant apps | Added `channel_auth_tenant` to `MicrosoftAppCredentials` | ✅ Right | Single Tenant bot apps require the tenant ID when fetching access tokens — without it the token endpoint returns the wrong format and `KeyError: 'access_token'` is thrown. Multi Tenant apps use a different token endpoint that doesn't need the tenant ID. |
| 2026-09-07 | Personal accounts and bot testing | Azure Bot Service bots require work/school M365 accounts | ⚠️ Constraint | Personal Microsoft accounts (`@outlook.com`) cannot interact with registered Azure Bot Service bots — Teams channel requires an M365 tenant. Used Azure Web Chat as the test surface instead. Real Teams testing requires an M365 Developer tenant or employer account. |
...
| 2026-09-07 | Week 2 Days 6-7 complete. Azure Bot Service registered (agilebot-dev, F0, Single Tenant). HMAC JWT signature verification added to Teams endpoint. ConnectorClient reply loop wired. ngrok tunnel established. Full bot loop verified via Azure Web Chat: message → routing → JIRA → reply. Next-gen JIRA sprint resolution fixed (Agile board API). Null priority fields fixed. PR pending. |
...
| Next-gen JIRA API compatibility | jira_tools.py | `openSprints()` JQL is unsupported in team-managed projects. Sprint resolution must go through the Agile board API. External APIs have undocumented project-type variants — always test against the actual instance, not the docs. |
| Null-safe API field access | jira_tools.py | Fields that look mandatory in the JIRA UI (priority, assignee) can be null in the REST API for next-gen projects. One unguarded key access silently breaks the entire tool result. Always use `.get()` with a fallback on nested API response fields. |
| Azure Bot Service Single Tenant auth | teams.py | Single Tenant bots require `channel_auth_tenant` in `MicrosoftAppCredentials` to fetch tokens from the correct endpoint. Without it the token response has no `access_token` key — a KeyError that only surfaces when the bot tries to send a reply. |
| Bot reply is async-decoupled | teams.py | The bot doesn't return its response in the HTTP response body. It sends a separate outbound POST to Azure via ConnectorClient after processing. This is the correct pattern for event-driven bots — the receiver and responder are decoupled. |
